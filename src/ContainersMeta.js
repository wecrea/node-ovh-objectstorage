...
class ContainersMeta {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (!(await this.context.containers().exist(container)))
          // noinspection ExceptionCaughtLocallyJS
          throw new Error("Container name specified in parameter don't exist.");

        let container_infos = await this.context.containers().info(container);
        let existant_headers = {};

        if (_.isUndefined(container_infos)) {
          existant_headers = _.map(container_infos.headers, (value, header) => {
            if (
              _.includes(_.toLower(header), _.toLower("X-Container-Meta-")) ||
  ...
  /**
   * Create many container meta
   *
   * @param {String} container Name of container
   * @param {Object} metas Key - value of meta
   *
   * @async
   * @return {Promise<Object>}
   */
  createMany(container, metas) {
    return new Promise(async (resolve, reject) => {
      try {
        // checks
        if (_.isUndefined(container))
          ...
          (err, res, body) => {
            ...
            let value = _.filter(res.headers, (value, header) => {
              return (
                ...
                _.toLower(
                  "X-Container-Meta-" +
                    _.toSlug(_.replace(_.toLower(key), /_/g, "-"))
                )
              );
            });
  ...
  has(container, key) {
    return new Promise(async (resolve, reject) => {
      try {
        resolve(
          !_.isNull(await this.context.containers().metas().get(container, key))
        );
      } catch (e) {
        return reject(e);
      }
