    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (!_.includes(path, "/"))
          // noinspection ExceptionCaughtLocallyJS
          throw new Error(
            "File path parameter isn't valid : container/filename.ext."
          );
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (
          ...
          // noinspection ExceptionCaughtLocallyJS
          throw new Error(
            "One or more file path are incorrect, paths type must be an array of string values."
          );
        if (
          _.count(
            _.filter(paths, (path) => {
          ...
          // noinspection ExceptionCaughtLocallyJS
          throw new Error(
            "One or more file path parameter isn't valid : container/filename.ext."
          );

        let deletes = await Promise.all(
          _.map(paths, (path) => {
  ...
  expire_at(path, expire_date) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        /** @type {Moment} **/
        expire_date = !moment.isMoment(expire_date)
          ? moment(expire_date)
          : expire_date;
        ...
        request(
          {
            ...
            headers: Object.assign(
              {
                "X-Auth-Token": this.context.token,
                "X-Delete-At": Math.round(
                  expire_date.toDate().getTime() / 1000
                ),
                Accept: "application/json",
              },
              existant_headers
